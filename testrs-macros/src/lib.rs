...
//!
//! The one exception is `#[test(cases(...))]`: its expression sources are
//! relocated into generated provider functions (see [`cases`]) so the CLI can
//! resolve them and so they type-check against the test's parameters.
//!
